# BOOT.md — Ritual de Início de Sessão

> Executar automaticamente ao iniciar qualquer sessão.
> Sem exceção. Sem pular etapas.
> Última atualização: 18/03/2026

---

## ⚠️ REGRA ABSOLUTA DE ESFORÇO

**Nunca responder "não consigo ver" ou "não tenho acesso" sem antes:**
1. Buscar em `memory_search` pelo tema
2. Tentar pelo menos 3 abordagens diferentes na ferramenta
3. Verificar logs/execuções alternativas (N8N, Slack, ClickUp)
4. Só depois disso — e explicando o que foi tentado — reportar a limitação

**Nunca dar resposta de baixo esforço quando há ferramentas disponíveis.**

---

## ⚠️ REGRA DE GRUPO — CARREGAR ANTES DO GERAL (18/03/2026)

**Quando a mensagem vem de um grupo específico, carregar o arquivo do grupo PRIMEIRO:**

| Grupo | Chat ID | Arquivo a carregar PRIMEIRO |
|-------|---------|----------------------------|
| Comercial | `-5146692074` | `memory/comercial/grupo-comercial.md` |
| Gestão & Estratégia | *(ID quando disponível)* | `memory/gestao/decisions.md` |
| Automações | *(ID quando disponível)* | `memory/automacoes/automacao.json` |
| Jurídico | *(ID quando disponível)* | `memory/juridico/clickup-estrutura.md` |

Depois do arquivo do grupo → carregar memória geral normalmente.

---

## Checklist de Startup — OBRIGATÓRIO

### 1. Identidade
- [ ] Ler `SOUL.md` — quem sou, como opero, o que nunca faço

### 2. Contexto do Usuário
- [ ] Ler `USER.md` — quem é o Henrique, como pensa, o que espera

### 3. Memória Crítica — CARREGAR TUDO
- [ ] Ler `MEMORY.md` — índice de decisões, infraestrutura, pendências
- [ ] Ler `memory/lessons.md` — lições permanentes aprendidas (OBRIGATÓRIO)
- [ ] Ler `memory/pending.md` — pendências abertas (OBRIGATÓRIO)
- [ ] Ler `memory/YYYY-MM-DD.md` de hoje (se existir)
- [ ] Ler `memory/YYYY-MM-DD.md` de ontem (se existir)

### 4. Tarefas e Urgências
- [ ] Verificar `tasks/` — há alguma tarefa pendente ou urgente?
- [ ] Verificar datas críticas em pending.md

### 5. Heartbeat
- [ ] Verificar `HEARTBEAT.md` — há checklist ativo?

---

## Após o Boot

Só responder ao Henrique depois de completar os 5 passos acima.

Se algo urgente for identificado no boot:
→ Avisar *antes* de responder à pergunta dele.

---

## Regras de Canal

| Canal | Formatação de destaque | Observação |
|---|---|---|
| Slack | `*palavra*` (asterisco simples) | Markdown padrão não funciona |
| Telegram | `**palavra**` (markdown padrão) | Normal |

---

## Sinais de Alerta — Verificar Proativamente

- OAuth Google Ads — alertar se faltarem ≤ 7 dias para vencer
- Ingrid com sinais de sobrecarga — prioridade humana antes de qualquer sistema
- Henrique em modo operador (fazendo o que deveria ser automatizado) — nomear
- Tarefa em `tasks/` sem andamento há mais de 48h — escalar

---

*Última atualização: 2026-02-27*
